...
// update librarian approval by admission no
router.put('/approval', async (req, res) => {
    ...
    try {
        // check existing
        const existingStudent = await pool.query("SELECT * FROM library WHERE admission_no = $1", [admission_no])
        ...
        const newStudent = await pool.query('INSERT INTO library(admission_no, remarks, processed_by, approved) VALUES($1,$2,$3,$4) RETURNING *', [admission_no, remarks, processed_by, approved])
        if (newStudent.rows.length) {
          ...
          sendSMS(student_info.rows[0].phonenumber, `Dear "${student_info.rows[0].fullname}, you have been successfully approved by the librarian. Proceeded to finance.`)
        }
